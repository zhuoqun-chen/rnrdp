...
ALGO_NAME = "BC_Diffusion_rgbd_UNet_DP"
...
import mani_skill2.envs  # type: ignore
import numpy as np
import torch
...
from gymnasium.wrappers.frame_stack import Box, FrameStack, LazyFrames
from mani_skill2.utils.wrappers import RecordEpisode  # type: ignore
from nets.cnn.plain_conv import PlainConv, PlainConv_MS1
from nets.dp.conditional_unet1d import ConditionalUnet1D
...
def parse_args():
    ...
    parser.add_argument("--demo-path", type=str, default='data/v0/rigid_body/StackCube-v0/trajectory.rgbd.pd_ee_delta_pos.h5',
    ...
    parser.add_argument("--num-inference-steps", type=int, default=100) # 1, 2, 4, 8, 100
...
class DictFrameStack(FrameStack):
    def __init__(
        ...
        for k, v in self.observation_space.items():  # type: ignore
...
def collect_episode_info(infos, result=None):
    ...
    if "final_info" in infos:  # infos is a dict
        ...
        for i in indices:
            ...
            ep = info["episode"]
            print(
                f"global_step={cur_iter}, ep_return={ep['r'][0]:.2f}, ep_len={ep['l'][0]}, success={info['success']}"
            )
